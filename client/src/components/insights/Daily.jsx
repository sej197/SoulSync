...
const FACTOR_QUIZ_MAP = {
  anxiety: {
    ...
    encouragement: "Understanding your anxiety patterns is the first step to feeling calmer.",
  },
  depression: {
    ...
    encouragement: "Checking in on your mood helps us support you better every day.",
  },
  stress: {
    route: "/stress-quiz",
      ...
      {dailyData.alerts?.length > 0 && (
        <div className="space-y-3">
          {dailyData.alerts.map((alert, i) => (
            <div key={i} className={`rounded-[2rem] p-6 border-2 flex items-start gap-4 transition-all ${getAlertStyle(alert.severity)}`}>
              <span className="text-3xl flex-shrink-0">{getAlertIcon(alert.type)}</span>
              <div className="flex-1">
                <h4 className="font-bold text-slate-800 mb-1 text-lg">{alert.message}</h4>
                <p className="text-sm text-slate-600 mb-3 leading-relaxed">{alert.detail}</p>
                {alert.action && (
        ...
        <div className="flex flex-col gap-6">
          <div className="group bg-gradient-to-br from-white to-purple-50 rounded-[2.5rem] p-10 shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 border border-purple-100 relative overflow-hidden flex-1">
            ...
            <div className="relative z-10 text-center">
              ...
              <h3 className="text-2xl font-serif text-[#4A148C] mb-8 font-bold">Daily Wellness Score</h3>

              <div className="relative w-64 h-64 mx-auto mb-8">
                <ResponsiveContainer width="100%" height="100%">
                  <RadialBarChart cx="50%" cy="50%" innerRadius="70%" outerRadius="100%" barSize={20}
                    data={[{ value: score, fill: getLevelColor(riskLevel) }]} startAngle={90} endAngle={-270}>
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  ...
                  <span className={`px-4 py-2 rounded-full text-xs font-bold mt-2 uppercase tracking-wide border-2 ${getRiskBadgeStyle(riskLevel)}`}>
                    {riskLevel === "LOW" ? "Doing Well" : riskLevel === "MODERATE" ? "Needs Attention" : "High Risk"}
                  </span>
                </div>
              </div>
              ...
              {dailyData.pattern_analysis && (
                <div className="flex gap-3 justify-center flex-wrap">
                  <div className="inline-flex items-center gap-2 px-4 py-2.5 rounded-2xl bg-[#FAFAFA] border border-gray-100">
                    ...
                    <span className={`text-xs font-bold ${dailyData.pattern_analysis.velocity_interpretation === "Improving" ? "text-emerald-600" : dailyData.pattern_analysis.velocity_interpretation === "Worsening" ? "text-red-600" : "text-slate-600"}`}>
                      {dailyData.pattern_analysis.velocity_interpretation}
                    </span>
                  </div>
                  <div className="inline-flex items-center gap-2 px-4 py-2.5 rounded-2xl bg-[#FAFAFA] border border-gray-100">
                    <span className="text-sm">🎭</span>
                    <span className="text-xs text-slate-500 font-medium">Stability</span>
                    <span className={`text-xs font-bold ${dailyData.pattern_analysis.mood_stability === "Stable" ? "text-emerald-600" : dailyData.pattern_analysis.mood_stability === "Moderate" ? "text-amber-600" : "text-red-600"}`}>
              ...
              <button onClick={onRefresh}
                ...
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 group-hover/btn:rotate-180 transition-transform duration-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          {assessmentCards.length > 0 ? (
            <div className="group bg-gradient-to-br from-white to-purple-50 rounded-[2.5rem] p-8 shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 border border-purple-100 relative overflow-hidden">
              ...
              <div className="relative z-10">
                <div className="flex items-center gap-3 mb-4">
                  <span className="p-2 bg-purple-100 rounded-xl border border-purple-200 text-lg">🎯</span>
                  <div>
                    <h3 className="text-[#4A148C] font-bold font-serif text-lg leading-tight">We noticed something</h3>
                    <p className="text-xs text-slate-400 mt-0.5">Quick check-ins to personalise your support</p>
                  </div>
                </div>
                <div className="space-y-3">
                  {assessmentCards.map((card, i) => (
                    <div key={i} className={`bg-gradient-to-r ${card.color} rounded-2xl p-4 border border-l-4 ${card.accentColor} hover:shadow-md transition-all duration-200`}>
                      <div className="flex items-center justify-between gap-3">
                        <div className="flex items-center gap-3 flex-1 min-w-0">
                          <span className="text-2xl flex-shrink-0">{card.icon}</span>
                          <div className="min-w-0">
                            <p className="font-bold text-slate-800 text-sm">{card.label}</p>
                            <p className="text-[11px] text-slate-500 truncate">{card.subtitle} — {card.encouragement}</p>
                          </div>
                        </div>
                        <button
            ...
            <div className="group bg-gradient-to-br from-white to-emerald-50 rounded-[2.5rem] p-8 shadow-lg border border-emerald-100">
              <div className="text-center py-4">
                ...
                <h3 className="font-bold text-[#4A148C] text-lg mb-2">You're on track!</h3>
                <p className="text-sm text-slate-500 leading-relaxed">
                  No areas of concern detected today. Keep maintaining your healthy habits.
                </p>
              </div>
            </div>
          )}
...
<div className="group bg-white rounded-[1.75rem] p-8 shadow-sm hover:shadow-md transition-all duration-500 relative overflow-hidden border border-slate-100">
  ...
  <div className="relative z-10">
    ...
    <div className="w-8 h-0.5 bg-purple-300 mx-auto mt-6 rounded-full" />
          ...
          {isHighRisk && (
            <div className="bg-gradient-to-br from-red-50 to-orange-50 rounded-[2rem] p-6 border-2 border-red-200 shadow-lg">
              <div className="flex items-start gap-3 mb-4">
                ...
                <div>
                  <h4 className="font-bold text-red-800 mb-1">You don't have to face this alone</h4>
                  <p className="text-xs text-red-700 leading-relaxed">
                    Reaching out is a sign of strength. Talk to someone right now.
                  </p>
                </div>
              </div>
              ...
              <button onClick={() => navigate("/support/helplines")}
              ...
              <div className="relative z-10">
                ...
                <div className="space-y-3">
                  {dailyData.insights.map((insight, i) => {
                    ...
                    return (
                      <div key={i} className={`rounded-2xl border ${accent.border} ${accent.bg} overflow-hidden hover:shadow-md transition-all duration-200`}>
                        <div className="flex">
                          <div className={`w-1.5 ${accent.strip} flex-shrink-0`} />
                          <div className="p-4 flex-1">
                            <div className="flex items-center gap-2 mb-1.5">
                              <span className="text-sm">{accent.icon}</span>
                              <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full ${accent.tagColor}`}>{accent.tag}</span>
                            </div>
                            <p className="font-bold text-sm text-slate-800 mb-1">{insight.message}</p>
      ...
      {/* ── SUGGESTIONS — visually rich cards ─────────────────────────────── */}
        ...
        <div className="group bg-gradient-to-br from-white to-amber-50 rounded-[2.5rem] p-8 md:p-10 shadow-lg hover:shadow-2xl transition-all duration-300 border border-amber-100 relative overflow-hidden">
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-3 mb-6">
              <span className="p-2.5 bg-gradient-to-br from-amber-100 to-orange-100 rounded-xl text-lg border border-amber-200 shadow-sm">💡</span>
              <div>
                <h3 className="text-[#4A148C] font-bold font-serif text-lg leading-tight">For You Today</h3>
                <p className="text-xs text-slate-400 mt-0.5">Personalised suggestions based on your check-ins</p>
              </div>
            </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <span className={`flex-shrink-0 w-8 h-8 rounded-xl bg-gradient-to-br ${accent.gradient} text-white text-xs font-bold flex items-center justify-center shadow-sm mt-0.5 group-hover/rec:scale-110 transition-transform duration-200`}>
                        {accent.icon}
                      </span>
                      <p className="text-[13px] text-slate-700 leading-relaxed font-medium flex-1">{rec}</p>
                    </div>
                  </div>
                );
              })}
      ...
      <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="relative z-10">
            <div className="flex justify-between items-center mb-4">
              <div className="flex items-center gap-2">
                <span className="p-1.5 bg-pink-100 rounded-lg border border-pink-200 text-sm">🎮</span>
                <h3 className="text-[#4A148C] font-bold text-lg font-serif">Wellness Activity</h3>
              </div>
              <button onClick={() => setCurrentActivity(rand(funActivities))}
            ...
            {currentActivity && (
              <div className="bg-white/70 p-5 rounded-2xl border border-pink-100 hover:bg-white transition-colors">
                <div className="flex items-center gap-4">
                  <span className="text-4xl bg-gradient-to-br from-pink-50 to-white p-3 rounded-2xl shadow-sm border border-pink-100">{currentActivity.icon}</span>
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <h4 className="text-[#4A148C] font-bold">{currentActivity.title}</h4>
                      <span className="text-[10px] bg-pink-100 text-pink-600 px-2 py-0.5 rounded-full font-bold">{currentActivity.duration}</span>
                    </div>
                    <p className="text-sm text-slate-600 leading-snug">{currentActivity.description}</p>
                  </div>
                </div>
              </div>
            )}
        ...
        <div className="group bg-gradient-to-br from-white to-blue-50 rounded-[2.5rem] p-8 shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 border border-blue-100 relative overflow-hidden">
          ...
          <div className="relative z-10">
            <div className="flex justify-between items-center mb-4">
              <div className="flex items-center gap-2">
                <span className="p-1.5 bg-blue-100 rounded-lg border border-blue-200 text-sm">🫁</span>
                <h3 className="text-[#4A148C] font-bold text-lg font-serif">Breathe & Reset</h3>
              </div>
              <button onClick={() => setCurrentBreathing(rand(breathingExercises))}
                className="text-xs text-[#7B1FA2] font-bold bg-purple-50 px-3 py-1.5 rounded-lg border border-purple-100 hover:bg-purple-100 transition-colors hover:scale-105">
                Next 🔀
              </button>
            </div>
            {currentBreathing && (
              <div className="bg-white/70 p-5 rounded-2xl border border-blue-100 hover:bg-white transition-colors">
                <div className="flex items-center gap-4">
                  <span className="text-4xl bg-gradient-to-br from-blue-50 to-white p-3 rounded-2xl shadow-sm border border-blue-100">{currentBreathing.icon}</span>
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <h4 className="text-[#4A148C] font-bold">{currentBreathing.name}</h4>
                      <span className="text-[10px] bg-blue-100 text-blue-600 px-2 py-0.5 rounded-full font-bold">{currentBreathing.duration}</span>
                    </div>
                    <div className="flex items-center gap-2 mb-1.5">
                      {currentBreathing.pattern.split('-').map((num, i) => (
                        <React.Fragment key={i}>
                          <span className="w-7 h-7 rounded-lg bg-blue-100 text-blue-700 text-xs font-bold flex items-center justify-center">{num}</span>
                          {i < currentBreathing.pattern.split('-').length - 1 && (
                            <span className="text-blue-300 text-xs">→</span>
                          )}
                        </React.Fragment>
                      ))}
                    </div>
                    <p className="text-sm text-slate-600 leading-snug">{currentBreathing.description}</p>
                  </div>
                </div>
              </div>
            )}
